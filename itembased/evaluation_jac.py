...
def evaluate(testdata, sim):
    ...
    for test_user in testdata:
        ...
        dist = jac_dist(recommended, removed)
...
def jac_dist(v1, v2):
    """
    Find the Jaccard distance between the given two vectors.
    vi is of the form [(colno., elem)].
    """
    v1r = set([i[0] for i in v1])
    v2r = set([i[0] for i in v2])
    return len(v1r.intersection(v2r)) / len(v1r.union(v2r))
