...
def find_norm(col):
    ...
    return (col[0], (col[1], norm))

def cos_sim(col_pair):
    """
    Find the cosine similarity of the given pair of columns.
...
if __name__ == '__main__':
    ...
    itemdata = rawdata.map(toColKey) \
    			...
    			.map(sort_col).map(find_norm)
    itempair = itemdata.cartesian(itemdata).filter(lambda (i1, i2): i1[0]<i2[0])
    cos_sim_pair = itempair.map(cos_sim)
    sim_data = cos_sim_pair.flatMap(extract_sim) \
               .aggregateByKey([], fill_col, assemble_col) \
               .map(sort_col_by_sim)
    sim_data.saveAsPickleFile('cos_sim')
