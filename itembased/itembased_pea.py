...
def find_norm(col):
    ...
    """
    avg_r = sum(i[1] for i in col[1]) / len(col[1])
    adjusted = [(i[0], i[1]-avg_r) for i in col[1]]
    norm = math.sqrt(sum([i[1]**2 for i in adjusted]))
    ret = [] if norm==0 else [(col[0], (adjusted, norm))]
    return ret

def pea_sim(col_pair):
    """
    Find the Pearson similarity of the given pair of columns.
...
if __name__ == '__main__':
    ...
    pea_sim_pair = itempair.map(pea_sim)
    sim_data = pea_sim_pair.flatMap(extract_sim) \
               .aggregateByKey([], fill_col, assemble_col) \
               .map(sort_col_by_sim)
    sim_data.saveAsPickleFile('pea_sim')
